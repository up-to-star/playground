...
__global__ void sgemmV6(const DType * __restrict__ A, const DType * __restrict__ B, DType * __restrict__ C, const int M, const int N, const int K) 
{
    ...
    for (int bk = 0; bk < (K + BK - 1) / BK; bk++) {
        ...
        #pragma unroll
        for (int k = 0; k < BK; k++) {
            WRITE_FLOAT4(r_comp_a[0]) = READ_FLOAT4(sa[k][ty * TM / 2]);
            WRITE_FLOAT4(r_comp_a[4]) = READ_FLOAT4(sa[k][ty * TM / 2 + BM / 2]);
            WRITE_FLOAT4(r_comp_b[0]) = READ_FLOAT4(sb[k][tx * TN / 2]);
            WRITE_FLOAT4(r_comp_b[4]) = READ_FLOAT4(sb[k][tx * TN / 2 + BN / 2]);
            #pragma unroll
            for (int m = 0; m < TM; m++) {
                #pragma unroll
                for (int n = 0; n < TN; n++) {
...
PLAYGROUND_MATMUL_DEC(float32_t, 6, M, N, K, A, B, C)
{
    ...
    sgemmV6<float32_t><<<gridDim, blockDim>>>(A, B, C, M, N, K);
}
}