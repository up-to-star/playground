...
{
__global__ void hgemm_v13(const float16_t* __restrict__ a,
                                 ...
                                 const int N, const int K)
{
    ...
    float16_t* s_a = smem;
    float16_t* s_b = smem + 2 * BM * (BK + APAD);
...
PLAYGROUND_MATMUL_SIG(float16_t, 13, M, N, K, A, B, C)
{
    ...
    cudaFuncSetAttribute(hgemm_v13,
                         cudaFuncAttributeMaxDynamicSharedMemorySize, 98304);

    unsigned int dsmem =
        2 * (BM * (BK + 8) + BK * (BN + 8)) * sizeof(float16_t);
    hgemm_v13<<<gridDim, blockDim, dsmem>>>(A, B, C, M, N, K);
}
}  // namespace playground